[bot.py](https://github.com/user-attachments/files/26811763/bot.py)
"""
...
async def handle_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Принимаем: "2.3 text", "2 3 text", "2. 3. text", "2,3 text" и т.д.
    m = re.match(r"^\s*(\d+)[.,;\s]+(\d+)[.,;\s]+(\S.*)$", text, flags=re.DOTALL)
    if not m:
...
def main():
    ...
    app.add_handler(
        MessageHandler(
            filters.TEXT & ~filters.COMMAND & filters.Regex(r"^\s*\d+[.,;\s]+\d+[.,;\s]+\S"),
            handle_edit,
        )
    )
