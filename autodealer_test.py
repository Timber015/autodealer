[autodealer_test.py](https://github.com/user-attachments/files/26842084/autodealer_test.py)
"""
...
# Селекторы — после анализа HTML АвтоДилера:
# на странице логина две формы (смена пароля + авторизация),
# нужно ограничить селектор конкретной формой .auth-form
LOGIN_SELECTORS = [
    'form.auth-form input.auth-email',
    'form.auth-form input[name="email"]',
    'input.auth-email',
]

PASSWORD_SELECTORS = [
    'form.auth-form input[name="password"]',
    'form.auth-form input[type="password"]',
]

SUBMIT_SELECTORS = [
    # Настоящая видимая кнопка — div со словом «Войти»
    'div.auth-form__btn',
    '.auth-form__btn.auth__btn',
    # Запасные варианты (скрытый submit)
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print("\n[3/4] Отправляю форму...")
        submitted = await try_click(page, SUBMIT_SELECTORS, "Кнопка входа")
        if not submitted:
            print("  → пробую Enter на поле пароля...")
            try:
                await page.locator('form.auth-form input[name="password"]').first.press("Enter")
                print("  ✅ Enter нажат")
            except Exception as e:
                print(f"  ❌ Enter не сработал: {e}")
        # Ждём загрузки / редиректа после логина — даём больше времени под AJAX
        try:
